...
$(document).ready(function(){
	...
	$(".wing").bind('change',function(){
		...
		$.get('ajax_add.php?q='+count+'&r='+id, function(data){
			content= data;
...
$('form#form2').submit( function(ev){
		...
		var e=$("#myTable table tbody tr:nth-child("+i+")  input[name=amount]").val();
		...
		$("#show").html('<div class="alert alert-error" style="color:red">'+response.text+'</div>')
			...
			if(response.report_type=='submit'){
				$(".rem").remove();
				$(".alert-success").show().append("<p>"+response.text+"</p><p><a class='btn green' href='menu.php'>ok</a></p>");
				//$("#output").remove();
			}
			$("html, body").animate({
			scrollTop:0
