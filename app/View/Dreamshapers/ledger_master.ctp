...
<div role="alert" aria-live="polite" class="toast-top-right" id="toast-container" style="display:none; padding-top:40px"><div style="" class="toast " id="hide"><button role="button"class="toast-close-button">×</button><div class="toast-title">ledger Master</div><div class="toast-message"> </div></div></div>
        ...
        <div class="tabbable tabbable-custom tabbable-border">
            <ul class="nav nav-tabs">
                <li <?php if(empty($active) || $active==1){?> class="active"<?php } else {?>class=""<?php }?>  >
                    <a aria-expanded="true" href="#tab_1_1" data-toggle="tab">Add Ledger Master
            ...
            <div class="tab-content">
				...
				?>
                    <form method="post" name="add">
                        ...
                        <td><div class="form-group"><label>Category</label></div></td>
                      	<td><div class="form-group"><select class="form-control input-medium" name="ledger_category_id" required>
                        ...
                        <tr>
                        <td colspan="6"><center><button name="add_ledger_master" value="add_ledger_master"  class="btn green"><i class="fa fa-plus"></i> Add</button></center></td>
                        </tr>
                        </table>
                     </div>
                    </form>
                ...
                <?php if(empty($fetch_ledger_master))
                {?>
    ...
    <tr>
     ...
     <?php
		$i=0;
		 ...
		 $ledger_category_id=$data['ledger_master'] ['ledger_category_id'];
		 ...
		 $ledger_category_name=$result_ledger_category[0]['ledger_category']['name'];
		 
         $name=$data['ledger_master'] ['name'];
		 ?>
        <tr>
        <td><?php echo $i;?></td>
        <td><?php echo $data['ledger_master'] ['name']; ?></td>
       <td><?php echo $ledger_category_name ?></td>
                        ...
                        <table class="table self-table" style=" width:50% !important;" border="0" align="center">
                         ...
                         <tr>
                         <input type="hidden" name="ledger_master_id" value="<?php echo $id; ?>" />
                        <td><div class="form-group"><label>Category</label></div></td>
                      	<td><div class="form-group"><select class="form-control input-medium" name="edit_ledger_category_id" required>
                                            <option value="">-- Select Category --</option>
                                            <?php
                                            foreach($fetch_ledger_category as $data)
                                            {
												$ff=$data['ledger_category']['id'];
												
                                            	?>
                                                <option value="<?php echo $data['ledger_category']['id'];?>" 
                                                <?php if($ledger_category_id==$ff){ echo 'selected'; }?>><?php echo $data['ledger_category']['name'];?></option>
                                                <?php
                                            }
                                            ?>
									</select></div>
          ...
          <input type="text" class="form-control" placeholder="Caretaker Name" value="<?php echo $name; ?>" name="edit_name" id="edit_name" align="left" required></td>
                        ...
                        <tr><td colspan="3"><center><div class="modal-footer">
											...
											<button type="submit" name="edit_ledger_master" value="edit_ledger_master" class="btn blue">Update</button>
										</div></center></td></tr>
                        </table>
										</div></form></div>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                             ...
                             <input type="hidden" name="delete_ledger_master_id" value="<?php echo $id; ?>" />

                            <button type="button" class="btn default" data-dismiss="modal">Cancel</button>
                             <button type="submit" name="delete_ledger_master" value="delete_ledger_master" class="btn blue">Delete</button>
 </form>
                        </div>
                    </div>
                    <!-- /.modal-content -->
