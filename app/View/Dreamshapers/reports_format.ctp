...
<div  class="hide_at_print">
        ...
        <tr><td colspan="4"><p align="center" style="background-color:#C33; color:#FFF"><strong>Reports</strong></p></td></tr>
				<td>
				...
				<td id="sub_ledger_ajax_view">
						<select class="form-control input-medium" name="ledger_cat_id" id="ledger_cat_id" required>
                                    <option value=""></option>
                          </select>
				</td>
...
$(document).ready(function(){
	   ...
	   $('select[name=ledger_category_id]').live('change',function(){
		...
		$.ajax({ 
			url: "ajax_php_file?receipt_payment_mode_fetch_report1=1&q="+ledger_category_id,
			success: function(data)
			{ 
				$("#ledger_cat_id").html(data);
			}
			});
	});
...
$(document).ready(function() {
	    ...
	    $("#go").live('click',function(){
				...
				var ledger_cat_id = $('#ledger_cat_id').val();
				var from=$('#date1').val();
				var to=$('#date2').val();
				$("#ledger_view").html('loading.....').load("ledger_report_ajax?ledger_master_id="+ledger_account_id+"&ledger_cat_id="+ledger_cat_id+"&from="+from+"&to="+to);
